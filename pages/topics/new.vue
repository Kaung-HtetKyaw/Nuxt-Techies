<template>
  <div>
    <create-topic type="create" collection="topic" :params="{data:topic}">
          ...
          <v-row dense>
            <v-col cols="12" sm="12" md="8" offset-md="2" class="white pa-4 pa-md-8 my-4 bs-border">
              <v-btn
                ...
                :loading="creating"
                class="white--text"
                @click="createTopic(writeFB)"
              >Create a Topic</v-btn>
            </v-col>
          </v-row>
        </v-container>
      </template>
  ...
  methods: {
    createTopic(callback) {
      ...
      if (isFile(this.file)) {
        ...
        return callback()
          .then((res) => {
            vm.$router.push({ name: "topics-id", params: { id: res.id } });
          })
